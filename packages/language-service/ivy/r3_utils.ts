...
export function findTightestTemplateNode(ast: r3.Node[], position: number): r3.Node|AST|undefined {
...
export class R3Visitor implements r3.Visitor {
  ...
  visit(node: r3.Node) {
    ...
    if (start <= this.position && this.position <= end) {
      // console.error(`Visiting ${node.constructor.name}, name = ${(node as any).name}, start = ${start}, end = ${end}`);
      this.path.push(node);
      node.visit(this);
    } else {
      // console.error(`Not visiting ${node.constructor.name}, name = ${(node as any).name}, start = ${start}, end = ${end}`)
    }
  }
...
export class ExpressionVisitor extends RecursiveAstVisitor {
  ...
  visit(node: AST, path: Array<r3.Node|AST>) {
    ...
    if (start <= this.position && this.position <= end) {
      ...
      // console.error(`Not visting ${node.constructor.name}, name = ${(node as any).name}, start = ${start}, end = ${end}`)
    }
  }
}
